	...
	return makeToolCard("game", function(tool) {
		...
		function updateFileMenu() {
			...
			tool.menu.push({
				...
				text: ".sms",
				icon: "download",
				description: "Sega Master System ROM: play on real hardware or an emulator"
			});
			tool.menu.push({
				control: "button",
				text: "export SMS ROM",
				icon: "download",
				description: "export game as a Sega Master System .sms ROM file",
				onclick: exportSmsRom
			});
			tool.menu.pop({ control: "group" });
		}
		...
		function exportSmsRom() {
			refreshGameData();
			// The Bitsy editor stores world data as top-level globals
			// (room, tile, sprite, item, dialog, palette, variable, tune, blip,
			//  drawings via renderer). We assemble a world-like object here so
			// SmsExporter does not depend on the editor's internal structure.
			var currentWorld = {
				room:     room,
				tile:     tile,
				sprite:   sprite,
				...
				tune:     tune,
				blip:     blip,
				// drawings are accessed per-id via the renderer
				getDrawing: renderer ? function(drwId) { return renderer.GetDrawingSource(drwId); } : function() { return null; },
			};
			var title = filenameFromGameTitle();
			if (typeof SmsExporter !== 'undefined') {
				SmsExporter.exportRom(currentWorld, title);
			} else {
				alert('SMS Exporter not loaded. Make sure sms-exporter.js is included.');
			}
		}
		function newGameDialog() {			var resetMessage = localization.GetStringOrFallback("reset_game_message", "Starting a new game will erase your old data. Consider exporting your work first! Are you sure you want to start over?");
